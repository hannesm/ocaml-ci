...
let install_dune_opam_lint ~cache ~network ~base =
  let open Obuilder_spec in
  stage ~from:base [
    run ~cache ~network "opam pin add -yn opam-dune-lint.dev https://github.com/ocurrent/opam-dune-lint.git#2f18f33203c8d9ff7679d9261883512f23fabc9d";
...
let opam_lint_spec ~base ~opam_files ~selection =
  ...
  let open Obuilder_spec in
  stage
    ~child_builds:["opam-dune-lint", install_dune_opam_lint ~cache ~network ~base]
    ~from:base @@
    comment "%s" (Fmt.strf "%a" Variant.pp selection.Selection.variant) ::
    user ~uid:1000 ~gid:1000 ::
    Opam_build.install_project_deps ~opam_files ~selection @ [
